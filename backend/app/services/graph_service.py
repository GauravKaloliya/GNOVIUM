...
class GraphService:
    ...
    def query_graph(self, workspace_id, relation_types=None, entity_type_ids=None, limit=200):
        ...
        if entity_type_ids:
            ...
            from sqlalchemy.types import UUID as SUUID
            import uuid
            ids = [uuid.UUID(eid) for eid in entity_type_ids]
            entity_q = entity_q.filter(Entity.entity_type_id.in_(ids))
    ...
    def traverse_graph(self, workspace_id, center_node_id, depth=2, relation_types=None):
        ...
        from app.models.domain import Entity
        import uuid
        entity_ids = [uuid.UUID(nid) for nid in visited_nodes]
